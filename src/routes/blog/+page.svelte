<script lang="ts">
    ...
    let blogsData: BlogRow[] = [];

    onMount(async () => {
        const { data, error }: PostgrestResponse<BlogRow> = await supabase.from("blogs").select("*")
        blogsData = data ?? []
    });

    const incrementClicks = async (id: string) => {
        const { data, error }: PostgrestResponse<BlogRow> = await supabase.from("blogs").select("*").eq("id", id)
        if(data != null) {
            const currentClicks = data[0].clicks
            if(currentClicks != null) {
                await supabase.from("blogs").update({ clicks: currentClicks+1 }).eq("id", id)
            }
        }
    }
</script>
...
<div class="min-h-[100vh] mb-[10rem]">
    ...
    <div class="w-full mt-[5rem] px-[10%]">
        <Title>Blog</Title>
        <p class="opacity-60">List of blogs I have written</p>
