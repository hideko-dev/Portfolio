...
<div class="flex justify-center md:items-center w-full h-screen font-writer-regular mb-[5rem] sm:mb-0">
    <div class="w-full md:w-max">
        ...
        <div class="grid grid-cols-1 w-full px-4 md:px-0 md:grid-cols-2 md:w-[45rem] lg:grid-cols-3 lg:w-[55rem] xl:grid-cols-4 gap-4 mt-6 xl:w-[70rem]" style="grid-template-rows: repeat(auto-fill, auto)">
            {#each blogs as blog, index}
                <DelayContent delay={index*50} href="./blogs/{blog.id}" class="border-[1px] border-border rounded-[3px] py-3 px-3 leading-[16px] cursor-pointer transition-all duration-150 hover:border-blue-500 hover:bg-blue-500 hover:bg-opacity-20 group">
